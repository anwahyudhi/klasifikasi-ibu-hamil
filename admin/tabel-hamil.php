 		...
 		<div class="panel-body">
 			<div class="table-responsive">
 				<table class="table table-hover table-bordered">
 					...
 					<tbody>
 						<?php
               $sql = "SELECT * FROM hasil_klasifikasi ";
          
 						                $no  = 1; 
                        foreach ($dbh->query($sql) as $data) :
                            if ($data['lingkar_lengan_atas_hasil'] <= 23.5) {
                                $kondisi = "KEK";
                                $kek++;

                            }
                            elseif($data['berat_badan_hasil'] <= 41 && $data['paritas_hasil'] <=2){
                               $kondisi = "KEK";
                               $kek++;
                            }
                            elseif ($data['hemoglobin_hasil'] <= 11 && $data['paritas_hasil'] <=2) {
                               $kondisi = "KEK";
                               $kek++;
                            }
                            elseif ($data['hemoglobin_hasil'] <= 11 && $data['berat_badan_hasil'] <= 41 && $data['paritas_hasil']<=2) {
                                $kondisi = "KEK";
                                $kek++;
                            }
                            else{
                        ...
                        <tr>
                        	...
                        	<td> <?php echo $no; ?> </td>
                        	<td> <?php echo $data['nama_ibu_hasil']; ?></td>
                            <td> <?php echo $data['lahir_ibu_hasil']; ?> Tahun</td>
                            <td> <?php echo $data['berat_badan_hasil']; ?> Kg</td>
                            <td> <?php echo $data['lingkar_lengan_atas_hasil']; ?> Cm</td>
                            <td> <?php echo $data['hemoglobin_hasil']; ?> dL</td>
                            <td> <?php echo $data['paritas_hasil']; ?></td>
